...
To summarize if L is a linear transformation, A is a matrix that represents L. There exists a matrix B such that AB = $I$, the L has an inverse $L^{-1}$ and B represents that Linear Transformation ($L^{-1}$) 
...
$$
A^{-1} = \frac{1}{det(A)}* \begin{bmatrix} a & -b \\ -c & d \end{bmatrix}
$$
...
A symmetric matrix \( S \) is defined as: $$ S = \begin{bmatrix} s_{11} & s_{12} & s_{13} & \cdots & s_{1n} \\ s_{12} & s_{22} & s_{23} & \cdots & s_{2n} \\ s_{13} & s_{23} & s_{33} & \cdots & s_{3n} \\ \vdots & \vdots & \vdots & \ddots & \vdots \\ s_{1n} & s_{2n} & s_{3n} & \cdots & s_{nn} \end{bmatrix} $$
## Square Matrix
